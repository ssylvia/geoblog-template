	...
	return function MovableGraphic(map, layer, graphic, onMoveEndCallback, onMoveStartCallback)
	{
		...
		function init()
		{
			...
			var event3 = on(layer._div.rawNode, touch.press, function(event) {
				...
				if (event.graphic == graphic || has("touch")) {
					map.disablePan();
			...
			var event4 = on(layer._div.rawNode, touch.release, function(event) {
				map.enablePan();
			...
			var event5 = has("touch") ? 
					// Using the layer decrease too much the performance ...
					on(map.__container, touch.move, moveGraphic)
